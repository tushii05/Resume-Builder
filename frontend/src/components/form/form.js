...
const FormComponent = ({ history }) => {
  ...
  const handleUserStrength = (e) => {
    setUserStrength(e.target.value);
  };

  //User Weakness
  const [userWeakness, setUserWeakness] = useState("");
  const handleUserWeakness = (e) => {
    setUserWeakness(e.target.value);
  };

  //User Hobbies Description
  const [userHobbiesDescribe, setUserHobbiesDescribe] = useState("");
  const handleUserHobbiesDescribe = (e) => {
    setUserHobbiesDescribe(e.target.value);
  };

  //User Strength Description
  const [userStrengthDescribe, setUserStrengthDescribe] = useState("");
  const handleUserStrengthDescribe = (e) => {
    setUserStrengthDescribe(e.target.value);
  };

  //User Hobbies Description
  const [userWeaknessDescribe, setUserWeaknessDescribe] = useState("");
  const handleUserWeaknessDescribe = (e) => {
    setUserWeaknessDescribe(e.target.value);
  };
  ...
  const handleUserResumeData = (e) => {
    ...
    axios
      .post("http://localhost:9999/api", {
        ...
        userWeakness,
        userHobbiesDescribe,
        userStrengthDescribe,
        userWeaknessDescribe
      })
      .then((res) => {
        if (res.data.success) {
  ...
  const Hobbies = async () => {
    ...
    await fetch("https://api.openai.com/v1/engines/davinci/completions", requestOptions)
      .then(response => response.text())
      .then(result => setUserHobbiesDescribe(JSON.parse(result).choices[0].text))
      // .then(result => result("q", JSON.parse(result).choices[0].text))
      .catch(error => console.log('error', error));
  }
  // console.log(userHobbiesDescribe, 'ggggg')
  ...
  const Strength = async () => {
    ...
    await fetch("https://api.openai.com/v1/engines/davinci/completions", requestOptions)
      .then(response => response.text())
      .then(result => setUserStrengthDescribe(JSON.parse(result).choices[0].text))
      // .then(result => result("q", JSON.parse(result).choices[0].text))
      .catch(error => console.log('error', error));
  }


  const Weakness = async () => {
    var myHeaders = new Headers();
    myHeaders.append("Authorization", "Bearer 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");
    myHeaders.append("Content-Type", "application/json");
    ...
    await fetch("https://api.openai.com/v1/engines/davinci/completions", requestOptions)
      .then(response => response.text())
      .then(result => setUserWeaknessDescribe(JSON.parse(result).choices[0].text))
      .catch(error => console.log('error', error));
  }
      ...
      <Form className="form-section">
        ...
        <Form.Row>
          ...
          <FormGroup as={Col} sm={12} md={6}>
            ...
            <FormControl
              type="text"
              placeholder="e.g university of the Madhya Pradesh"
        ...
        <Form.Row>
          <FormGroup as={Col} sm={12}>
            <Form.Label className="Hello">Hobbies</Form.Label>
            <Form.Control
              placeholder="Hobbies Type"
          ...
          <FormGroup as={Col} sm={12}>
            <FormLabel>Describe your Hobbies</FormLabel>
            <Form.Control
              as="textarea"
              rows={6}
              placeholder="Describe your Hobbies"
              onClick={() => Hobbies()}
              onChange={handleUserHobbiesDescribe}
              value={userHobbiesDescribe}
              required
            />
          </FormGroup>
        </Form.Row>
        <Form.Row>
          <FormGroup as={Col} sm={12}>
            <Form.Label className="Hello">Strength</Form.Label>
            <Form.Control
              placeholder="Strength Type"
              onChange={handleUserStrength}
              value={userStrength}
              required
            />
          </FormGroup>
          <FormGroup as={Col} sm={12}>
            <FormLabel>Describe your Strength</FormLabel>
            <Form.Control
              as="textarea"
              rows={6}
              placeholder="Describe your Strength"
              onClick={() => Strength()}
              onChange={handleUserStrengthDescribe}
              value={userStrengthDescribe}
              required
            />
          </FormGroup>
        </Form.Row>
        <Form.Row>
          <FormGroup as={Col} sm={12}>
            <Form.Label className="Hello">Weakness</Form.Label>
            <Form.Control
              placeholder="Weakness Type"
              onChange={handleUserWeakness}
              value={userWeakness}
              required
            />
          </FormGroup>
          <FormGroup as={Col} sm={12}>
            <FormLabel>Describe your Weakness</FormLabel>
            <Form.Control
              as="textarea"
              rows={6}
              placeholder="Describe your Weakness"
              onClick={() => Weakness()}
              onChange={handleUserWeaknessDescribe}
              value={userWeaknessDescribe}
              required
            />
          </FormGroup>
        </Form.Row>
